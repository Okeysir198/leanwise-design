...
**Reading this file.** It starts at `v1.1.0`, the repository replacement — everything before it
is summarised in the `[0.x]` section at the foot and lives in `git`. Two consumers are still
pinned inside that older range; `CLAUDE.md` §Consumers is the enumerated pin table.
...
- **Surface tiers no longer read as patches on the page.** The light lifts
  `--lw-surface-1..3` and the hairlines `--lw-border-1/2` were hue 45 at 14–20% saturation
  and 2.5–7.1% darker than the page: a beige on a neutral white, which is exactly what a
  mismatched background looks like. Every subtle-ground component — sidebar, table `th`,
  segmented shell, KPI, filter chip, `code`, user bubble, menu and row hover — showed as a
  visibly different colour from the screen around it. The ramp moves to hue 38 (the same
  paper, off the olive edge of yellow) and tightens to 1.6/3.5/5.8% below the page:
  `#FCFBFA` `#F7F6F5` `#F2F1EE` `#EBEAE7` `#D9D7D3`. The page (`--lw-surface-0`), every text
  tier, `--lw-line-control`, the shadow ink (already hue 30) and every dark value are
  unchanged. All tiers moved lighter, so no text ratio regressed — the `--lw-text-3` floor
  is now 4.75 on surface-3, up from 4.61.
- **shadcn `--card` / `--popover` on light pointed at the wrong tier.** Both resolved to
  `--lw-bg-subtle-c` while `--background` resolved to `--lw-bg-c`, directly contradicting the
  comment above them ("the same as the page on light, and LIFT on dark"), so every shadcn
  Card, Popover, Dialog and Sheet painted a tinted ground on an untinted page. They now
  resolve to `--lw-bg-c`; the dark block's lift is untouched.
- `email.css` literals follow the new ramp. The backdrop and inset panel take the muted tier
  (`#F7F6F5`) instead of subtle, which at 1.6% off white would render as no frame at all.
- **Template literals had their own grey family.** `PitchDeck` and `Email` painted grounds, rules
  and muted ink from a cool blue-grey set no token accounts for — `#F6F8FB` `#F6F8FA` `#DDE3EC`
  `#E3E8EF` `#CBD3DE` `#42506A` `#4A5567` `#6B7A90` `#8894A8` `#6B7684` — beside slides and shells
  painted `#FFFFFF` from the same files. Both now track the system's own literals. This is the
  `email.css` drift of v3.0.0 in a second location: a literal is a second home, and the only
  thing that keeps one honest is a comparison in both directions.
- New specimen card, `preview/colors-surfaces.html` (Colors → "Surfaces on the page"): the four
  light grounds edge to edge on the page they sit on, with a rail / flush card / table head mock.
  The ramp defect above was invisible on every existing card because no card put a tinted ground
  and the page ground in one frame — the tiers were only ever shown as a swatch row.

  The card is painted from the **tiers** (`--lw-surface-*`, `--lw-text-*`), which are
  theme-invariant, so the stage shows light paper under a dark theme as well — that is the
  subject. Painting it from the ROLES was the first attempt and axe failed it at once: the
  grounds re-pointed to navy while the tier inks stayed navy, 11 nodes of navy-on-navy. It is
  the trap CLAUDE.md names — a tier is theme-invariant, a role re-points — and the card that
  demonstrates the tiers is the one place mixing them is guaranteed to bite.

### Visual

- **4 of 184 shots moved, and they are all one card.** `preview/colors-neutrals` × light/dark ×
  comfortable/compact each moved 0.0432% (712 px, 461 strongly), which is the printed hex
  captions under its surface swatches re-rendering as different glyphs — the identical pixel
  count in light and dark is the evidence, since a ground tint would differ between them. Two
  further shots moved 0.0001%, below tolerance. **Nothing else moved at all**: the retune is
  2–3/255 per channel, under the Δ>8 floor, which is the measurable statement of why a defect
  this visible was invisible to a pixel gate. The four new `colors-surfaces` shots were recorded,
  not compared. Head commit carries `[visual-ok]`.

### Deprecated

- **`email.css` and the `Email` template — removed in v4.0.0.** The layer cannot use logical
  properties (mail clients), so it is a physical-property island in a package that is otherwise
  fully logical; `check:contrast` asserts its literals and nothing asserts its layout; and those
  literals have now drifted twice, in v3.0.0 (the file) and v3.1.5 (the template). No product in
  the org sends transactional mail today. The recipe survives in this file and in REVIEW.md if one
  starts. Migration: copy `email.css` into the sending product — an email stylesheet wants to live
  beside the templates it styles, not in a design system that cannot gate it.
- **The `pitch-deck` template — removed in v4.0.0.** 2,969 of its lines are a vendored deck
  scaffold carrying its own dark palette in Claude's coral `#D97757`, marked
  `@ds-adherence-ignore` on line 1 and exempt from the adherence lint, and the header says
  re-copying it overwrites the file — so a local fix is lost silently and tokenising it is undone
  by the next upstream copy. Migration: build the deck in the consuming project from the starter
  there, and take the type and colour from `tokens.css` like any other surface.
...
## [0.x] — the pre-1.1.0 tree

**Not reproduced here.** Every tag from `v0.1.0` to `v0.9.0` documented a repository that
`v1.1.0` replaced wholesale, and these entries were themselves reconstructed after the fact by
diffing those tags — a second home for a history `git` already holds. Read them at the source
when a pinned consumer needs them:

```bash
git show v0.9.0:CHANGELOG.md
```

The two facts from that range that still matter are kept where they are acted on rather than
here: six of those versions are breaking (**0.1.4** dependency URL, **0.2.0** removal, **0.7.0**
**0.8.0** **0.9.0** visual/palette), and `v0.2.2` ships a `package.json` reporting `0.2.1`, so a
pin there resolves correctly to a tree reporting the older number — REVIEW.md §8 carries the
consumer bump plan.
