...
export function Pagination({
  ...
  pageSizes = [25, 50, 100], cursor, hasNext, hasPrev, label = "Pagination",
  prevLabel = "Previous page", nextLabel = "Next page", pageSizeLabel = "Rows per page",
  ...
  return (
    <nav className={cx("lw-pagination", className)} aria-label={label} {...rest}>
      <span className="lw-pag-info">
        {cursor ? formatCursor(page) : total ? formatCount(from, to, total, (v) => nf.format(v)) : ""}
      </span>
      <span className="lw-spacer" />
      {onPageSizeChange && !cursor && (
        <select className="lw-input lw-input-sm lw-pag-size" aria-label={pageSizeLabel}
          value={pageSize} onChange={(e) => onPageSizeChange(Number(e.target.value))}>
          {pageSizes.map(s => <option key={s} value={s}>{formatPageSize(s)}</option>)}
        </select>
      )}
      <button type="button" className="lw-pag-btn" aria-label={prevLabel}
      ...
      {!cursor && pages(page, count).map((p, i) => p === "gap"
        ...
        : <button key={p} type="button" className="lw-pag-btn" aria-label={formatPageLabel(p)}
            aria-current={p === page ? "page" : undefined} onClick={() => go(p)}>{p}</button>
      )}
      <button type="button" className="lw-pag-btn" aria-label={nextLabel}
