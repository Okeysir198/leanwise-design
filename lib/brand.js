...
const NAVY_INK = "#0B1220";
const NAVY_INK_LUM = 0.00556; // luminance([11, 18, 32])
 ...
 *
 * This measures both and returns the winner rather than testing a hardcoded
 * threshold. The old form was `luminance > 0.35 ? navy : white`, but the real
 * crossover is 0.190 — so every fill between the two picked WHITE when navy was
 * the readable choice. It never bit while brand-500 was a light cyan (luminance
 * 0.36, safely above), and would have the moment v0.8.0 darkened it to 0.135.
 * A tenant colour landing in that band would have failed AA silently.
 *
 * Both inks can still fall short on a mid-luminance fill (the worst case sits
 * near 4.2:1). brandVars() clamps tenant colours out of that band before calling
 * this; the LeanWise tokens themselves are gated by bin/lw-contrast-check.mjs.
 */
export function inkOn(hex) {
  const rgb = hexToRgb(hex);
  if (!rgb) return "#FFFFFF";
  const l = luminance(rgb);
  return ratio(l, NAVY_INK_LUM) >= ratio(l, 1) ? NAVY_INK : "#FFFFFF";
}
