...
export interface BrandVars extends Record<string, string> {}
...
export function hexToHslTriple(hex: string, dark = false): string | null {
  const rgb = hexToRgb(hex);
  if (!rgb) return null;
 ...
 */
export function brandVars(accent?: string | null, dark = false): BrandVars {
  if (!accent) return {};
