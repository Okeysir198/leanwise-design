...
```bash
npm install
npm run check          # contrast + self-check + theme completeness + barrel types.
...
**CI runs all six** (`.github/workflows/ci.yml`, every push/PR). Local `npm run check` is
deliberately the four that need no browser.
...
templates/_tooling/  the six gates + _css.mjs (the shared CSS reader) + tsup.config.js. Note the PATH assumption: ROOT is TWO
...
_ds_bundle.js     compiled browser bundle + card index, namespace `LeanWiseDesign_f2d907`.
                    GENERATED in the design project — there is no generator here. The cards
                    render from THIS, not from components/*.jsx, so a .jsx fix is invisible
                    to check:a11y until the bundle catches up. v1.1.3 mirrored two ARIA fixes
                    into it by hand; that is a stopgap, not the pattern. Re-pull the bundle
                    with the next wholesale sync.
...
## The six gates — all must stay green
...
- **`lw-visual.mjs`** — every card x light/dark x comfortable/compact. ⚠️ **This gate cannot
  ...
  records 136 fresh shots and compares nothing. The gate now says so out loud instead of
  printing "no visual change". It is genuinely useful locally for a before/after within one
  session. To make it real in CI, record the baselines **inside the CI image** — do not
  commit this box's set, which would make CI permanently red rather than green.
