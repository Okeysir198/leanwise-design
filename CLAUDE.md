...
package.json already saying `v3.0.1`. Its own `ds:contract` caught it via `.ds-pin-sha`; nothing in a
plain `npm install` would have. `rm -rf node_modules/@leanwise/design` then install the tag explicitly.

⚠ **Two things to steal from `leanwise-inspect` when bumping the rest.** Its `ds:presence` re-checks
