...
`npm run check:tokens` runs the lint's **CSS self-check** (`--css`) — the raw-duration and
raw-z-index rules over `base.css` / `marketing.css` / `product.css`. The TSX rules (raw hex,
