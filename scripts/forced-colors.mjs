 ...
 *
 *   node scripts/forced-colors.mjs
 *   node scripts/forced-colors.mjs --self-test   # prove both failure modes are caught
 */
import { readFileSync } from "node:fs";
import { join } from "node:path";
import { pathToFileURL } from "node:url";
import { ROOT } from "./lib/generated.mjs";
...
try { ({ chromium } = await import("playwright")); }
  ...
  console.error("forced-colors: needs playwright. `npm i -D playwright && npx playwright install chromium`.");
  process.exit(1);
}
...
try {
  if (process.argv.includes("--self-test")) {
    ...
    console.log("forced-colors --self-test: both failure modes are detectable");
    process.exit(0);
  }

  /* Written beside marketing.css so its relative url()s resolve, then removed. */
  const { writeFileSync, rmSync } = await import("node:fs");
  const tmp = join(ROOT, ".forced-colors.html");
  ...
  process.exit(report("forced-colors", {
    problems, checked, minChecked: 8,
    summary: `forced-colors OK — ${WRAPPERS.length} ground wrapper(s) survive forced-colors with every decorative pseudo-element hidden (${checked} readings)`,
